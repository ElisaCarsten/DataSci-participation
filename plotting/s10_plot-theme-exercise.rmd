...
airquality2 <-
  airquality %>% 
  mutate(Month2 = factor(Month,
                           levels = c(5, 6, 7, 8, 9),
                           labels = c("May", "June", "July", "August", "September")))

ggplot(airquality2, aes(`Month2`, `Temp`, group = fct_rev(`Month2`))) +
    geom_boxplot(aes(fct_rev(`Month2`)), outlier.shape = NA) +
    geom_jitter(alpha = 0.3, color = "blue") +
    labs(x = "",
         y = "",
         title = "Maximum temperature by month") +
    theme_bw() + 
    annotate("text", x = 4.08, y = 95, label="°F", size = 16) +
