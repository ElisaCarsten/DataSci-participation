...
gapminder %>% 
  ...
  geom_point(shape = 21, colour = "blue", fill = "#40b8d0") +
  scale_size_area(name = "Population", labels = scales::comma_format()) +
...
```r
ggplot(gapminder) +
  geom_point(aes(gdpPercap, lifeExp), shape = 20, size = 1) +
...
tribble(
  ...
  ggplot(aes(x, y)) + 
  geom_path(arrow = arrow(length = unit(0.15, "inches"))) +
  geom_text(aes(label = time), nudge_x = 0, nudge_y = 0.045)
```
