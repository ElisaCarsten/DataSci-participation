...
```
Continent is a factor with 5 levels (Africa, Americas, Asia, Europe, and Oceania). Asia is represented by the number 3.
...
h_countries <- c("Peru", "Spain", "Italy", "Ireland", "Lebanon")
...
```
Country has 142 levels. After filtering to the 5 countries, there are still 142 levels.
...
h_gap_dropped$country %>% 
  nlevels()
```
There are now 5 levels for country in the filtered dataset.
